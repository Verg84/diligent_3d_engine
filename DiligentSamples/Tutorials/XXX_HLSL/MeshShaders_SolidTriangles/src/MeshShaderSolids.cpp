#include"MeshShaderSolids.hpp"

namespace Diligent
{
SampleBase* CreateSample()
{
    return new MeshShaderSolids();
}
void MeshShaderSolids::CreatePipelineMeshShader()
{
...
void MeshShaderSolids::Initialize(const SampleInitInfo& InitInfo)
{
    SampleBase::Initialize(InitInfo);
    CreatePipelineMeshShader();
}
void MeshShaderSolids::Update(double ctime, double etime)
{
    SampleBase::Update(ctime, etime);
}
void MeshShaderSolids::Render()
{
    ...
    DrawMeshAttribs drawMesh;
    drawMesh.ThreadGroupCount = 1;
    drawMesh.Flags=DRAW_FLAG_VERIFY_ALL;

    m_pImmediateContext->DrawMesh(drawMesh);
